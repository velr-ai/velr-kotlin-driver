...
/**
 * Streaming result-table handle returned by {@link Velr#exec(String)}.
 *
 ...
 * produced by it.
 */
public final class Stream implements AutoCloseable, ChildHandle, ParentHandle, Iterable<Table> {
    private long handle;
    private final ParentHandle parent;
    private final HandleSet children = new HandleSet();

    Stream(long handle, ParentHandle parent) {
        this.handle = handle;
        this.parent = parent;
    }
    ...
    public Table nextTable() {
        long table = Native.streamNext(ptr());
        if (table == 0) {
        ...
        Table out = new Table(NativeSupport.requireHandle(table, "stream returned null table"), this);
        registerChild(out);
        return out;
    }
    ...
    private long ptr() {
        if (handle == 0) {
            throw new VelrException("Velr stream is closed");
        }
        return handle;
    }
}
